...
MindBridge 是面向校园心理场景的多 Agent 智能体平台，核心能力包括：学生端 SSE 流式聊天、Basic Auth 角色隔离、LangGraph 多 Agent 编排（含自研 runtime 兜底）、动态路由 RAG（Chroma 向量库 + BM25 + 本地 reranker）、心理风险评估、后台报告与高风险预警闭环、MCP 工具服务、异步工具队列、RAG 评测与工程 Harness。
...
Agent runtime 工厂。根据 `settings.agent_framework` 选择具体实现：`event_driven_multi_agent` → `EventDrivenAgentRuntimeService`；`langgraph` 且 langgraph 已安装 → `LangGraphAgentRuntimeService`；否则回退到 `AgentRuntimeService`。`agent_framework_status()` 暴露 requested/active/langgraphAvailable/fallback 状态供 API 使用。

### `app/agents/runtime.py`
自研有限循环 runtime（兜底实现）。定义 `AgentStep`、`AgentContext`（共享状态：意图、风险、记忆摘要、检索知识、回复消息列表等）、`AgentRunResult`，以及 `AgentRuntimeService`（`max_steps=8`）。`run()` 顺序尝试 MemoryAgent → SupervisorAgent → KnowledgeAgent → RiskGuardianAgent → CompanionAgent → CounselorAgent，由各 Agent 通过布尔返回值判断是否本轮执行。包含：
- `MemoryAgent`：从 Redis 加载短期记忆，回填 MySQL，生成本轮记忆摘要。
- `SupervisorAgent`：意图分类（CHAT/CONSULT/RISK），含硬信号兜底。
- `KnowledgeAgent`：把学生输入改写为检索 query 并执行 RAG。
- `RiskGuardianAgent`：调用 LLM JSON 评估，含高风险词硬兜底。
- `CompanionAgent`：CHAT 路径回复方案。
- `CounselorAgent`：CONSULT/RISK 路径回复方案，注入知识上下文与 skill 指引。

### `app/agents/langgraph_runtime.py`
LangGraph 实现的 bounded agent loop，继承自 `AgentRuntimeService`。构建 `StateGraph`，节点为 controller/memory/supervisor/knowledge/risk_guardian/companion/counselor，controller 通过 `_select_next_agent` 条件边选择下一个 Agent，所有非 controller 节点执行后回到 controller。受 `recursion_limit = max_steps * 3 + 2` 约束。

### `app/agents/event_driven_runtime.py`
事件驱动多 Agent runtime。基于 `CollaborationBlackboard` 与 `EventDrivenCoordinator` 运行 `UnderstandingAgent`、`SafetyAgent`、`ContextAgent`、`ResponseAgent`，由 `CoordinatorAgent` 维护任务板。`run()` 创建 blackboard、发布 TURN_STARTED 事件、交给 coordinator 运行；`_to_result()` 把 blackboard 上的 artifacts/events/tasks 转回 `AgentRunResult`，含意图选择、风险最高级取值、SAFETY_OVERRIDE 提升为 HIGH 等逻辑。

### `app/agents/autonomous.py`
事件驱动多 Agent 协作中的自主 Agent 实现。包含：
- `AgentRuntimeServices`：注入 db/settings/user/session/ai/model_registry/memory/private_memory/knowledge。
- `AgentPrivateMemory`：按 `agent:{name}:{session}` 隔离的 Redis 私有记忆。
- `BaseAutonomousAgent`：提供 `client()`、`private_memory()`、`remember()`、`_artifact()` 通用能力。
- `UnderstandingAgent`：意图分类与主题识别，发布 `intent` artifact。
- `SafetyAgent`：独立风险评估（含 SAFETY_OVERRIDE）与候选回复安全审查；高风险时强制叠加安全处理。
- `ContextAgent`：聚合记忆摘要、模型历史、RAG 检索、skill 上下文，发布 `context` artifact。
- `ResponseAgent`：根据意图/风险/上下文提出候选回复方案（normal_chat 或 support 模式），发布 `response_proposal`。
- `CoordinatorAgent`：维护任务板、预算、安全门槛、最终采纳；不参与固定 Agent 顺序。

### `app/agents/coordinator.py`
`EventDrivenCoordinator` —— claim-based 协调器。负责：root task 创建、按轮次（`agent_max_rounds`）派发任务、每轮限 `agent_max_claims_per_round` 个 claim、单 Agent 限 `agent_max_claims_per_agent` 个 claim、最终采纳需 SafetyAgent 审查通过且置信度 ≥ `agent_final_acceptance_min_confidence`。`_derive_missing_work()` 根据当前 artifacts 推导缺失任务（intent/risk/context/response_proposal/safety_review/revise-response），`_claim_candidates()` 按优先级与置信度排序选取候选。

### `app/agents/events.py`
事件驱动协作的数据结构。`AgentEventType`、`TaskStatus`、`TaskPriority`、`AgentTask`、`AgentMessage`、`AgentArtifact`、`AgentEvent`、`AgentTurnResult`、`CollaborationBlackboard`（append-only 黑板，提供 `add_task`/`update_task`/`append_event`/`send_message`/`add_artifact`/`apply_turn_result`/`open_tasks`/`artifacts_by_kind`/`latest_artifact`/`accept_final` 等方法）。所有数据结构均为 `frozen=True` 的 dataclass。

### `app/agents/registry.py`
Agent 注册表与协议。`AgentCapability` 枚举、`AgentProfile`（名称、能力、system prompt、记忆策略、模型 profile、工具权限）、`AgentDecision`（claim/confidence/reason）、`AutonomousAgent` Protocol（`decide`/`act`）、`AgentCandidate`、`AgentRegistry`（按能力匹配任务、按置信度排序候选）。
...
`AgentTraceService.save_run` —— 把一次 Agent run 的输入、记忆摘要、agent steps（含协作事件/任务/artifact）、检索知识、回复消息、评估结果序列化为 JSON 写入 `agent_run_traces` 表。
...
- `test_event_driven_multi_agent.py`：验证黑板 append-only、artifact 不互相覆盖、registry 按置信度排序、coordinator 通过 claim 派发任务、`AgentModelRegistry` 支持 per-Agent 模型覆盖。
    ...
    -> app/api/routes.py (路由)
      -> app/services/chat.py (SSE 流式聊天)
        -> app/agents/harness.py (MindBridgeAgentHarness)
          -> app/agents/factory.py (选择 runtime)
            -> event_driven_runtime / langgraph_runtime / runtime
